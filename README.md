...
> ⚠️ **代理冲突提示**：如果本机开着代理软件（Clash / V2Ray 等），Python 进程请求 `localhost:8000` 可能被代理拦截返回 502。
> 前端默认已使用 `http://127.0.0.1:8000` 直连，若仍失败，请把 `localhost`、`127.0.0.1` 加入代理软件的白名单（绕过规则），或临时关闭代理。
...
| 问题 | 解决 |
| --- | --- |
| 前端报「无法连接后端」/ 502 | 后端未启动，或代理拦截了 `localhost`。先用 `uvicorn src.main:app --reload --port 8000` 启动；前端默认用 `http://127.0.0.1:8000` 直连，若仍失败请把 `localhost`、`127.0.0.1` 加入代理白名单 |
| 开代理时后端连接异常 | 本地开发建议设 `API_BASE=http://127.0.0.1:8000`（前端已默认），并让代理软件绕过本地回环地址 |
| 启动辩论报 429 | 触发频控（每 IP 每分钟 5 次），等一分钟再试 |
| 数据库表报错 | 删除项目根目录 `cyber_foodie.db`，重启后端自动重建 |
| 侧边栏排查连接 | 点「🔧 测试后端连接」按钮，会显示请求 URL、状态码、耗时 |
